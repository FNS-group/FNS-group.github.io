...
- [第 6 章 - 浮点](#第-6-章---浮点)
  - [6.1 浮点表示法](#61-浮点表示法)
    ...
    - [6.1.2 IEEE 浮点表示法](#612-ieee-浮点表示法)
      - [IEEE 单精度](#ieee-单精度)
...
为了简化硬件, 采用固定的格式来存储浮点数. 这种格式类似科学计数法:     
规范的浮点数:    
...
4. e 和 f 的某些组合有特殊含义:  
   ...
   - $e = 0\ and\ f \neq 0$. 表示一个非规范数.  
   - $e = FF\ and\ f = 0$. 表示无穷大($\infty$), 包括正无穷大和负无穷大.  
...
### 6.3 数学协处理器
#### 6.3.1 硬件
早期的 Intel 处理器并未提供支持浮点操作的硬件, 浮点操作需要由许多非浮点指令组成. 早期系统 Intel 提供了一片额外的被称为数学协处理器(math coprocessor)的芯片, 相对非浮点指令组成的浮点运算程序, 数学协处理器执行浮点操作的速度会快至少十倍.  
8086/8088 的协处理器是 8087, 80286 的是 80287, 80386 的是 80387, 80486DX 内置. 

数学协处理器有八个浮点数寄存器, 每个寄存器总是存储 80 位扩展精度浮点, 它们分别是 ST0 到 ST7. 与主 CPU 不同的是, 数学协处理器的浮点数寄存器被看作一个堆栈进行管理. 数学协处理器也有一个状态寄存器, 有 4 个标志位, $C_0, C_1, C_2\ and\ C_3$.   


#### 6.3.2 指令
所有协处理器助词符都以 F 开头.  
##### 导入和存储
**FLD *source***, 从内存导入一个浮点数到栈顶, *source* 可以是单精度/双精度/扩展精度浮点数或一个协处理器寄存器.  
**FILD *source***, 从内存中读一个整数, 转成浮点后推入栈中. *source* 可以是字/双字/四字.  
**FLD1**, 将 1 存储到栈顶.   
**FLDZ**, 将 0 存储到栈顶.   

**FST *dest***, 将栈顶的值(ST0)存储到内存中. *dest* 可以是单精度/双精度数或一个协处理器寄存器.  
**FSTP *dest***, 完成和 FST 相同的操作以外, 将 ST0 弹出.  
**FIST *dest***, 将栈顶值转换为整型后存储. *dest* 可以是字/双字.  
**FISTP, *dest***, 参考上面两个.  

**FXCH, ST*n***, 将 ST0 的值与 ST*n* 的交换. n 取 1 到 7.  
**FFREE ST*n***, 释放寄存器.  

##### 加法和减法
**FADD *src***, ST0 += *src*. *src* 可以是任何协处理器寄存器或者内存中的单精度/双精度.  
**FADD *dest*, ST0**, *dest* += ST0. *dest* 可以是任何协处理器寄存器.  
**FADDP, *dest* or**, 通上, 完成后 ST0 弹出.  
...
segment .text
    ...
    fldz                      ; ST0 = 0
lp:
    fadd qword [esi]          ; STO += *(eso)
    add esi, 8
    loop lp

    fstp qword sum
```
注意 FADD 需要指定内存操作数是单精度(双字大小)还是双精度(四字大小).  

**FSUB *src***, ST0 -= *src*.  
**FSUBR *src***, ST0 = *src* - ST0.  
**FSUB *dest*, ST0**, *dest* -= ST0.  
**FSUBR *dest*, ST0**, *dest* = ST0 - *dest*.  
上面四个指令尾部加一个 P, 类比加法, 为完成相同操作后将 ST0 弹出.  
**FISUB *src***, ST0 -= (float) *src*.  
**FISUBR *src***, ST0 -= (float) *src* - ST0.  

减法的十个指令中 *src* 的含义全部类比加法部.  

##### 乘法和除法
乘法和加法完全类似, 将加法中所有的 ADD 换成 MUL, 所有加号换成乘号就完成了.  

除法和减法, 与乘法和加法同理. 注意可以除以 0, 结果是无穷量.  

##### 比较
**FCOM *src***, 比较 ST0 和 *src*. *src* 可以是协处理器寄存器或者内存中的单精度/双精度.  
**FCOMP *src***, 比较后弹出.  
**FCOMPP**, 比较 ST0 和 ST1, 然后双双弹出.  
**FICOM *src***, 比较 ST0 和 (float) *src*. *src* 是内存中的字或双字.  
**FICOMP *src***.  
**FIST**, 比较 ST0 和 0.  

这些比较会改变协处理器的状态寄存器. CPU 无法访问, 但是有指令可以将它传递到 FLAGS 相同的比特位上:  
**FSTSW *dest***, 存储协处理器状态字到内存的一个字或 AX 寄存器中.  
**SAHF**, 将 AH 寄存器中的值存储到 FLAGS 寄存器中.  
...
;
    fld qword [x]        ; ST0 = x
    fcomp qword [y]      ; 比较 ST0 和 y, 然后弹出来, 也就是比较 x 和 y
    fstsw ax             ; 将 $C_n$ 状态标志位存储到 FLAGS 中
    sahf
    jna else_part        ; if !(x > y), 但是要注意这里用的是无符号整数的跳转指令
then_part:
    ; then 部分的代码
    jmp end_if
else_part:
...
dmax:
    ...
    fld qword [d2]
    fld qword [d1]       ; ST0 = d1, ST1 = d2, last-in on the top
    fcomip st1           ; d1 已经弹出来了
    jna short d2_bigger
d1_bigger:
    fcomp st0            ; 这句不是为了比较, 而是把 d2 弹出来
    fld qword [d1]       ; 再把 d1 加载进去
    jmp short exit
d2_bigger:
    ; 因为上面用的是 fcomip, 所以 d2 已经在栈顶了
    ; 翻译版又有错误, 还好我聪明
exit:
    leave
    ret
...
##### 杂项指令
**FCHS**, $ST0 = -ST0$, 改变 ST0 的符号位.  
**FABS**, $ST0 = |ST0|$, 求绝对值.  
**FSQRT**, $ST0 = \sqrt{ST0}$, 求平方根.  
**FSCALE**, $ST0 = ST0 \times 2^{\lfloor ST1 \rfloor}$.  

### 6.3.3 样例
### 6.3.4 二次方程求根公式
### 6.3.5 从文件中读数组
### 6.3.6 查找素数
不得不说这三个样例都是非常精彩的. 但是受篇幅限制(指太懒), 以后有缘再补上吧.  
...
从汇编的角度看, 结构体无非就是拥有不同大小元素的数组. 关注下面这个 C 结构体:  
```c
struct S {
    short int x;   // 2 字节整数
    int       y;   // 4 字节整数
...
```
有个内存段的图的, 但是不是必须要的.  
首先, ANSI C 标准规定结构体中的元素在内存中存储的顺序要和在结构体定义中的顺序一样, 并且第一个元素要恰好在结构体偏移量为 0 的地址处, aka 起始地址.  
在标准 C 头文件 `stddef.h` 中定义了一个宏 `offsetof()`,  它可以用来计算和返回结构体中任意元素的偏移地址, 比如 `offsetof(S, y)` 的结果是 2.  

#### 7.1.2 内存地址对齐
好了, 如果上面的 `offsetof(S, y)` 的用 gcc 跑得到的结果会是 4 而不是 2. 和其它很多编译器一样, gcc 在缺省情况下, 变量对齐在双字界上, 也就是 4 字节. 在 32 位保护模式下, 如果数据是从双字界开始存储的, CPU 则可以快速访问内存.  
如果要使用 C 和汇编的接口技术, 就需要约定好它们的偏移地址. gcc 提供了一个方案:  
```c
typedef short int unaligned_int __attribute__((aligned(1)));
```
这样可以让 `unaligned_int` 类型按 $2^1$ 个字节对齐, 也就是字.  
如果把 `x` 的类型换为 `unaligned_int` 类型, `y` 的偏移量确实会变为 2, 但是双精度依然会按缺省的双字对齐, 也就是 8.  
可以做的事是, 为结构体定义添加一个标注:  
```c
struct S {
    short int x;   // 2 字节整数
...
Microsoft 和 Borland 的编译器支持使用 `#pragma` 指示符指定对齐方式:  
```c
#pragma pack(1)
```
中间的 1 可以用 2, 4, 8 或者 16 代替, 分别代表对齐的字节边界(压缩), 字边界, 双字边界, 四字边界, 节边界.  
这个 pack 还支持 push 和 pop, 见下面的代码:  
```c
...
```
这样能在文件之间套 `#include` 的时候避免一些奇怪的问题. 关于是什么奇怪的问题请自行脑补.  

#### 7.1.3 位域s
```
string S {
    unsigned f1 : 3;   // 3 个位域
    unsigned f2 : 10;  // 10 个位域
    unsigned f3 : 11;  // 11 个位域
    unsigned f4 : 8;   // 8 个位域
};
```
位域指定的比特位大小不一定要是 8 的倍数. 比如上面的代码定义了一个 32 位的变量. 好了, 问题来了, 由于不是 8 的倍数, 有些变量跨了字节界. 于是在它们的存储上就要花点功夫了, 对于 big endian 来说一切还正常, 但是对于 little endian 处理器来说, 这些字节就会显得异常混乱, 不过实际上只要反转所有字节就能使它们重新结合.  

关于位域具体有什么用, 书中拿 SCSI(Small Computer Systems Interface)举例. 我觉得这辈子可能都用不上, 所以就不提了, 并且原文也没有什么可以精简的话, 所以直接见原文.  

#### 7.1.4 在汇编语言中使用结构体
在汇编语言中访问结构体跟数组是相同的, 反正不论是数组的 offset 还是结构体的 offset 都是人算出来的, 所以对于汇编来说区别反而不大.   
然后就是说为啥要用结构体指针参数代替结构体参数. 拿结构体作数值传递给函数通常是坏注意, 结构体一般都较大, 以数值传递时, 所有的数据都必须复制到堆栈中, 效率极低.  
很显然, 结构体变量基本上不可能存到 EAX 寄存器中返回. 编译器普遍采用的解决方法是在内部重写函数, 让它携带一个结构体指针参数, 返回时将值直接返回到指针中.  

说了这么多, 最后一段属实非常精彩:  
> 大多数汇编器(包括 NASM)都有在你的汇编代码中定义结构体的内置支持. 查阅你的资料来得到更详细的信息.   
